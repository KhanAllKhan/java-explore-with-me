...
public class PrivateRequestController {
    ...
    public RequestDto addRequest(@PathVariable(value = "userId") @Min(0) Long userId,
    ...
    @GetMapping
    public List<RequestDto> getAllRequests(@PathVariable(value = "userId") @Min(0) Long userId) {
        log.info("GET запрос на получение всех запросов на участие в событиях пользователя с id= {}", userId);
        return requestService.getRequestsByUserId(userId);
    }

    @PatchMapping("/{requestId}/cancel")
    public RequestDto canceledRequest(@PathVariable(value = "userId") @Min(0) Long userId,
